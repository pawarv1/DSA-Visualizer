...
export class DHHashTable {
    ...
    protected primary_hash(key: number) {
        return ((key % this.capacity) + this.capacity) % this.capacity;
    }

    protected secondary_hash(key: number) {
        const mod = this.capacity - 1;
        return 1 + (((key % mod) + mod) % mod);
    }
    ...
    async add(context: CanvasRenderingContext2D, key: number, fadeTime: number = 1): Promise<boolean> {
        ...
        let h1 = this.primary_hash(key);
        let h2 = this.secondary_hash(key);
        let i = h1;
        let state = this.buckets.getStateAt(i);
        let firstTombstone = -1;

        for (let j = 0; j < this.capacity; j++) {
            i = (h1 + j * h2) % this.capacity;
